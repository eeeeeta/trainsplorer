...
pub fn get_train_for_rid_uid_ssd<T: GenericConnection>(conn: &T, rid: String, uid: String, start_date: NaiveDate) -> Result<Train> {
    ...
    let trains = Train::from_select(conn, "WHERE EXISTS(SELECT * FROM schedules WHERE uid = $1 AND id = trains.parent_train) AND date = $2", &[&uid, &start_date])?;
    if trains.len() > 1 {
...
pub fn process_ts<T: GenericConnection>(conn: &T, ts: Ts) -> Result<()> {
    ...
    for (tiploc, action, time, tstd) in updates {
        ...
        let tmvts = TrainMvt::from_select(conn, "WHERE parent_mvt = $1 AND source != 0", &[&mvt.id])?;
        for tmvt in tmvts {
